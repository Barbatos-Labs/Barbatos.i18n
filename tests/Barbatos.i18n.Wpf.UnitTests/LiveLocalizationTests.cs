...
public sealed class LiveLocalizationTests : IDisposable
{
    ...
    {
        var converter = new StringLocalizerConverter("price", null, "", null, hasCultureSlot: true, keyFromBinding: false);
    ...
    {
        var converter = new StringLocalizerConverter(null, null, "", null, hasCultureSlot: true, keyFromBinding: true);
    ...
    [Fact]
    public void Convert_ShouldReadTheKeyFromTheBoundValue_WhenThereIsNoCultureSlot()
    {
        var converter = new StringLocalizerConverter(null, null, "", null, hasCultureSlot: false, keyFromBinding: true);
        var values = new object[] { "greeting", "Hùng" };
    ...
    [Fact]
    public void PluralConvert_ShouldUseEnumMemberNamesAsKeys()
    {
        ...
        var values = new object[] { new CultureInfo("vi-VN"), SampleFruit.Apple, SampleFruit.Apples, 4 };

        var result = converter.Convert(values, typeof(string), null!, CultureInfo.CurrentCulture);

        result.Should().Be("Nhiều quả táo (4)");
    }
    ...
    [Fact]
    public void Convert_ShouldAlignPerArgumentStringFormatsWithTheArgumentsThatFollowTheReservedSlots()
    {
        var converter = new StringLocalizerConverter("greeting", null, "", new string?[] { "<{0}>" }, hasCultureSlot: true, keyFromBinding: false);
        var values = new object[] { new CultureInfo("vi-VN"), "Hùng" };
    ...
    [Fact]
    public void PluralConvert_ShouldFallBackToTheFormThatWasSupplied()
    {
        var converter = new PluralStringLocalizerConverter(
            "apple", null, null, "", hasCultureSlot: true, keyFromBinding: false, pluralKeyFromBinding: false, staticCount: 9);
        var values = new object[] { new CultureInfo("vi-VN") };

        var result = converter.Convert(values, typeof(string), null!, CultureInfo.CurrentCulture);

        result.Should().Be("Một quả táo (9)");
    }
    ...
    public void PluralExtension_NonLivePath_ShouldFallBackToText_WhenPluralTextIsMissingAndCountIsPlural()
    {
        ...
        result.Should().Be("Một quả táo (5)");
    }
    ...
    public void PluralExtension_NonLivePath_ShouldFallBackToPluralText_WhenTextIsMissingAndCountIsSingular()
    {
        ...
        result.Should().Be("Nhiều quả táo (1)");
    }
    ...
    [Fact]
    public void LocalizationSource_Refresh_ShouldRaisePropertyChanged()
    {
        LocalizationSource source = LocalizationSource.Instance;
        int raised = 0;
